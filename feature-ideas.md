...
- ~~Company search says "can't find companies" even though it shows up — misleading.~~ — complete on 2026-07-21: reworded the empty-state text on both the Search page and the Home page's company preview (they were saying "no curated companies match... use the search box above" directly beneath the live dropdown that already found a real match, e.g. QQQ — confusing since it's the same box). Now explains the curated list and the live any-ticker dropdown are two different things and points at the dropdown result directly.
